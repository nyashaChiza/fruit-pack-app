...
export default function DriverOrderListScreen() {
  ...
  if (loading) return <Text className="p-4 text-center text-gray-400">Loading assigned orders...</Text>;

  return (
    <SafeAreaView className="flex-1 bg-green-50">
      <View className="flex-1 ">
        <Text className="text-2xl font-bold text-green-900 p-4 text-center">Assigned Orders</Text>
        {orders.length === 0 ? (
          <Text className="text-center text-gray-500 mt-4">No assigned orders at the moment.</Text>
        ) : (
          <FlatList
            data={orders}
